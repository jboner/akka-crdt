...
class ChangeListenerSpec 
  ...
  "A subscriber to the event stream" must {
   	"receive events about (all types of) CvRDT changes" in {
   		...
   		// g-counter
   		val gc1 = storage.getOrCreate[GCounter]("jonas").get
      ...
      storage.update(gc2)
		  expectMsgType[GCounter](10 seconds)
   		...
   		// pn-counter
		  val pnc1 = storage.getOrCreate[PNCounter]("jonas").get
      ...
      storage.update(pnc3)
		  expectMsgType[PNCounter](10 seconds)
   		...
   		// g-set
   		val gs1 = storage.getOrCreate[GSet]("jonas").get
      ...
      storage.update(gs2)
   	  ...
   	  // 2p-set
   		val tps1 = storage.getOrCreate[TwoPhaseSet]("jonas").get

...
CmRDTs require a reliable broadcast channel. This can be implemented either through a single highly-available replicated event log, or (perhaps more interesting) letting each replica have its own event log as a form of sharding in order to avoid introducing a [SPOF](http://en.wikipedia.org/wiki/Single_point_of_failure) (single point of failure) and SPOB (single point of bottleneck). This approach also opens up for optimisations like storing the snapshot and replaying from the snapshot etc., as in classic [Event-Sourcing](http://martinfowler.com/eaaDev/EventSourcing.html). 
